...
func AuthorizedResourceControlDeletion(resourceControl *api.ResourceControl, context *RestrictedRequestContext) bool {
	if context.IsAdmin {
		return true
	}
...
func AuthorizedResourceControlUpdate(resourceControl *api.ResourceControl, context *RestrictedRequestContext) bool {
	userAccessesCount := len(resourceControl.UserAccesses)
...
func AuthorizedResourceControlCreation(resourceControl *api.ResourceControl, context *RestrictedRequestContext) bool {
	if context.IsAdmin {
		return true
	}
...
func AuthorizedTeamManagement(teamID api.TeamID, context *RestrictedRequestContext) bool {
	if context.IsAdmin {
		return true
	}

	for _, membership := range context.UserMemberships {
		if membership.TeamID == teamID && membership.Role == api.TeamLeader {
			return true
		}
	}
...
func AuthorizedUserManagement(userID api.UserID, context *RestrictedRequestContext) bool {
