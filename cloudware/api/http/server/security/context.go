...
func storeTokenData(request *http.Request, tokenData *api.TokenData) context.Context {
...
func RetrieveTokenData(request *http.Request) (*api.TokenData, error) {
	contextData := request.Context().Value(contextAuthenticationKey)
	if contextData == nil {
		return nil, api.ErrMissingContextData
	}

	tokenData := contextData.(*api.TokenData)
...
func RetrieveRestrictedRequestContext(request *http.Request) (*RestrictedRequestContext, error) {
	...
	if contextData == nil {
		return nil, api.ErrMissingSecurityContext
	}
